#!/usr/bin/env python3
"""Add 'Inputs & Outputs' section to all Course 04 notebooks that don't have it."""
...
def main():
    ...
    for r in added:
        print(" ", r)
    print("\nSkipped (already have section):", len(skipped))
    print("Skipped (no code cell):", len(no_code))
    for r in no_code:
        print(" ", r)
    print(f"\nTotal: {len(added)} added, {len(skipped)} skipped, {len(no_code)} no code, {len(failed)} failed.")
